...
def get_module():
    ...
    result = questionary.select(
        ...
        choices=[
            ...
            Choice(f"{next(counter)}) Mint Ethereum Year Badge [Canvas]", mint_eth_badge),
            Choice(f"{next(counter)}) Mint OmniHub Badge [Canvas]", mint_omnihub_badge),
            Choice(f"{next(counter)}) Mint Trusta Labs Reputation Badge [Canvas]\n", mint_trustalabs_badge),
